...
from utils_kernel import fspecial_gauss
...
# --------------------------------------------
|--model_zoo                # model_zoo
   |--latest                # model_name, optimized for PSNR
|--testsets                 # testsets
   |--real_set              # testset_name, contain 2 real images
|--test_log                 # results
...
def main():
    ...
    # Preparation
    # ----------------------------------------
    model_name = 'latest'      # 'usrgan' | 'usrnet' | 'usrgan_tiny' | 'usrnet_tiny'
    ...
    k = fspecial_gauss(25, kernel_width)
    k = util.shift_pixel(k, sf)  # shift the kernel
    k /= np.sum(k)
    ...
    print('model_name:{}, image sigma:{}'.format(model_name, noise_level_img))
    print(L_path)
    ...
    w, h = img_L.shape[:2]
    print('{:>10s}--> ({:>4d}x{:<4d})'.format(img_name+ext, w, h))

    # boundary handling
    boarder = 8     # default setting for kernel size 25x25
    img = cv2.resize(img_L, (sf*h, sf*w), interpolation=cv2.INTER_NEAREST)
    img = util.wrap_boundary_liu(img, [int(np.ceil(sf*w/boarder+2)*boarder), int(np.ceil(sf*h/boarder+2)*boarder)])

    img_wrap = img[0::sf, 0::sf, ...]
    img_wrap[:w, :h, :] = img_L
    img_L = img_wrap
